...
- `asct report network`: View network interfaces, addresses, and namespace information
...
Use the `asct report network` command to inspect network interfaces and address configuration.
...
### What `asct report network` collects

Use `asct report network` to report networking configuration. It includes the following information:
...
### View `asct report network` output
...
Example output for `asct report network`:
