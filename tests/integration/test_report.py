...
# csv_report_csv = ["system-info.csv", "network-info.csv", "cmn.csv"]
csv_report_csv = ["system-info.csv", "network.csv"]
json_report_content = ["system-info", "network", "cmn"]
...
def test_report_view_matches_output(test_work_dir, output_format):
    ...
    assert path.isdir(path.join(report_dir, "raw", "network"))
    if output_format == "stdout":
