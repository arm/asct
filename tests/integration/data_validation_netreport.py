...
def validate_json_data(checked_json: dict) -> None:
    """Validate the network report JSON produced by `asct system-info --format json`.

    The JSON report is expected to contain a top-level `network` key.
    ...
    net = checked_json["network"]
    assert isinstance(net, dict)

    # net_local
    net_local = net.get("net_local") or {}
    _require_keys(net_local, ["local_ipv4", "local_ipv6", "aliases"], "network[net_local]")
    ...
    assert len(net_dev) >= 1, "Expected at least one interface in network.net_dev"
    ...
    for idx, dev in enumerate(net_dev):
        assert isinstance(dev, dict), f"network.net_dev[{idx}] is not a dict: {type(dev)}"
        _require_keys(dev, required_dev_keys, f"network[net_dev][{idx}]")
        is_str(dev["name"], f"network[net_dev][{idx}][name]")
    ...
    for idx, ns in enumerate(net_ns):
        assert isinstance(ns, dict), f"network.net_ns[{idx}] is not a dict: {type(ns)}"
        # Best-effort keys (can vary by env)
        _require_keys(ns, ["ns", "name", "pid", "command", "error", "devices"], f"network[net_ns][{idx}]")
...
def validate_csv_data(json_report: dict, csv_report: list[list[str]]) -> None:
    ...
    assert "network" in json_report, "JSON report missing top-level 'network' key"
    network_root = json_report.get("network") or {}
