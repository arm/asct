...
cmn_api_module = importlib.import_module("asct.core.cmn.cmn_api")
CMN = cmn_module.CMN
...
def test_detect_cpus_suppresses_tool_output_and_validates_mappings(monkeypatch, capsys):
    debug_messages = []

    def detect(_args):
        print("Discovered CPUs:")
        print("detection diagnostic", file=cmn_api_module.sys.stderr)

    monkeypatch.setattr(cmn_api_module.cmn_detect_cpu, "main", detect)
    monkeypatch.setattr(cmn_api_module.cmn_json, "cmn_config_filename", lambda: "cmn-system.json")
    monkeypatch.setattr(cmn_api_module.log, "debug", debug_messages.append)
    monkeypatch.setattr(
        cmn_api_module,
        "_system_from_json_file",
        lambda **_kwargs: SimpleNamespace(has_cpu_mappings=lambda: True),
    )

    cmn_api_module.detect_cpus()

    captured = capsys.readouterr()
    assert not captured.out
    assert not captured.err
    assert "Discovered CPUs:" in debug_messages[0]
    assert "detection diagnostic" in debug_messages[0]
