...
from asct.core.managers.ubench_reporter import get_reporter as get_ubench_reporter
...
def test_iperf3_graph_output_uses_current_benchmark_dir_for_error_rows(monkeypatch):
    calls = []
    reporter = SimpleNamespace(
        output_dir="unused",
        plot_iperf3_network_usage=lambda _df, filename, protocol_label=None: calls.append((
            "network",
            filename,
            protocol_label,
        )),
    )
    monkeypatch.setattr(iperf3_mod, "get_reporter", lambda: reporter)
    ...
    recipe.rows = [
        {
            ...
            "status": "error",
            "error_message": "iperf3 failed",
        }
    ]

    df = recipe.get_results_df()

    assert calls == [
        ("network", "iperf3-udp-sweep_network_usage.png", "UDP"),
    ]
    assert df.loc[0, "status"] == "error"
    assert df.loc[0, "error_message"] == "iperf3 failed"


def test_iperf3_graph_output_skips_error_rows_without_metrics(monkeypatch, tmp_path):
    reporter = get_ubench_reporter()
    previous_output_dir = reporter.output_dir
    previous_current_benchmark = reporter.current_benchmark
    reporter.output_dir = str(tmp_path)
    reporter.current_benchmark = "iperf3-udp-sweep"
    monkeypatch.setattr(iperf3_mod, "get_reporter", lambda: reporter)
    ...
    recipe.rows = [
        {
            ...
            "sender_mbps": None,
            "receiver_mbps": None,
            "sender_cpu_total_pct": None,
            "receiver_cpu_total_pct": None,
            "sender_cpu_user_pct": None,
            "sender_cpu_system_pct": None,
            "receiver_cpu_user_pct": None,
            "receiver_cpu_system_pct": None,
            "status": "error",
            "error_message": "iperf3 failed",
        }
    ]
    ...
    try:
        ...
        reporter.output_dir = previous_output_dir
        reporter.current_benchmark = previous_current_benchmark
