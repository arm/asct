...
class HugePageManager:
    ...
    @thread_safe
    def request_allocation(self, requestor, request_info, disable_resize):
        """
        ...
        self._update_alloc_status_with_request(requestor, request_info, disable_resize)

    def _adjust_to_available_size(self, node, size_kb, count, disable_resize):
        ...
        scales the page count to cover the same total memory.
        ...
        """
        available_sizes = self._available_page_sizes.get(node, [])
        if size_kb in available_sizes:
    ...
    def _update_alloc_status_with_request(self, requestor, request_info, disable_resize):
        if self._alloc_done:
        ...
        for node, sizes in request_info.items():
            ...
            for page_size_in_kb, count in sizes.items():
                adjusted_size, adjusted_count = self._adjust_to_available_size(
                    node, page_size_in_kb, count, disable_resize
                )
    ...
    @classmethod
    def _read_hugepage_info(cls, node, size_kb):
    ...
    def _allocate_huge_page(cls, node_idx, size_kb, initial_count, added_count):
        ...
        if cls._read_hugepage_info(node_idx, size_kb) != count:
            raise MemoryError(
    ...
    def _get_hugepage_settings(self):
        ...
        for node, node_data in self._alloc_status.nodes.items():
            for size_kb, entry in node_data.sizes.items():
                try:
                    allocated_count = self._read_hugepage_info(node, size_kb)
    ...
    def _preverify(self):
        for node_idx, node_data in self._alloc_status.nodes.items():
            node_path = self._get_hugepage_node_path(node_idx)
            ...
            for size_kb in node_data.sizes:
                sysfs_path = self._get_hugepage_sysfs_path(node_idx, size_kb)
        ...
        for node_idx, node_data in self._alloc_status.nodes.items():
            for size_kb, size_data in node_data.sizes.items():
                ...
                for page_count in page_counts:
                    try:
                        self._allocate_huge_page(node_idx, size_kb, size_data.initial_count, page_count)
                    except (MemoryError, OSError, ValueError) as exc:  # noqa: PERF203
                        self._set_request_state(
                            node_idx,
                            size_kb,
                            HugePageAllocState.FAILED,
    ...
    def release(self, requestor):
        ...
        for request in self._alloc_status.all_requests[requestor_id]:
            ...
            if not size_info.requests and size_info.allocated_count > size_info.initial_count:
                try:
                    self._write_hugepage_info(node, size_kb, size_info.initial_count)
...
class HugepageImpl(Resource):
    ...
    def __init__(self, request_info):
        super().__init__()
        self.depends_on = ["CheckSudo"]
        HugepageImpl._huge_page_manager.request_allocation(self, request_info, AGS().disable_hugepage_resize)
...
def Hugepage(requested_size):
    ...
    return HugepageImpl(requested_size)
