...
def print_cached_dependencies(cached_dependencies):
    """Print cached dependency results to be used by benchmarks"""
    if not cached_dependencies:
        return

    print("Using cached dependency results:")
    for dep, dependents in cached_dependencies.items():
        print(f"  {dep} (required by {', '.join(dependents)})")
    print()
...
def run(args):
    ...
    with ctx as global_lock:
        ...
        try:
            ...
            write_asct_json_header(args, filtered_recipes.complete_list, user_config)
            log.critical(f"Failed to gather system information: {exc}")
            sys.exit(1)

        # Validate the system cache before deciding whether cached dependencies
        # can be reused. refresh_cache() may remove all recipe caches when the
        # system configuration has changed.
        filtered_recipes.complete_list = run_recipe_registry.resolve_dependencies(filtered_recipes.complete_list)
        ...
        if not args.quiet:
            print_cached_dependencies(filtered_recipes.cached_dependencies)
