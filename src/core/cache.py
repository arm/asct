...
class ASCTCache(metaclass=ASCTSingleton):
    ...
    def __init__(self, use_cache=False, output_folder="./"):
        ...
        self.is_cache_writeable = None
        if self.use_cache:
            log.debug("ASCT cache is enabled and will be used where possible")
            self.create_asct_cache_dir()
            self.is_cache_writeable = self._is_cache_directory_writeable()
        else:
            self.is_cache_writeable = False
    ...
    def _is_cache_directory_writeable(self):
        cache_dir = self.cache_directory
        if os.path.isdir(cache_dir) and os.access(cache_dir, os.W_OK):
            return True
        log.warning(f"ASCT cache directory {cache_dir} is not writeable, cache writes will be skipped")
        return False

    def _cache_write_enabled(self, operation):
        if self.is_cache_writeable:
            return True
        log.debug(f"ASCT cache directory is not writeable, skipping {operation}")
        return False
    ...
    def create_asct_cache_dir(self):
        ...
        """
        if self.is_cache_writeable is False:
            return
    ...
    def clear_asct_cache(self, invalidate=False):
        ...
        """
        if not self._cache_write_enabled("cache invalidation"):
            return
    ...
    def save_cache_validator(self, system_info):
        ...
        if not self._cache_write_enabled("cache validator save"):
            return
    ...
    def save(self, recipe):
        ...
        if not self.use_cache:
            return
        if not self._cache_write_enabled("recipe cache save"):
            return
    ...
    def clear_recipe_cache(self, recipe_name: str):
        ...
        if not self.use_cache:
            ...
            return
        if not self._cache_write_enabled(f"cache clear for {recipe_name}"):
            return
    ...
    def restore_to_output_folder(self, recipe_name: str) -> bool:
        """
    ...
    def write_to_cache(self, recipe_data: dict, recipe_name: str):
        ...
        """
        if not self._cache_write_enabled(f"cache write for {recipe_name}"):
            return
