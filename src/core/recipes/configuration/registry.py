...
class RecipeRegistry:
    ...
    def resolve_dependencies(self, benchmark_list: list) -> list:
        """
        Returns a list of dependencies required by the specified benchmarks,
        excluding those already present in the benchmark_list and a dict that maps
        every dependency to the benchmarks that use it.
        """
        dependencies = set()
        rev_dependencies = {}
        # allow access to global recipes so that we can resolve dependencies
        # that are not present in the current self._recipes.
        for benchmark in benchmark_list:
            # Ensure benchmark does not have higher priority (lower value) than any of its dependencies
            for dep in self._global_recipes[benchmark].depends_on:
                if dep not in rev_dependencies:
                    rev_dependencies[dep] = []
                rev_dependencies[dep].append(benchmark)
            # Add dependencies of the benchmark to the set
            dependencies.update(self._global_recipes[benchmark].depends_on)

        # Exclude benchmarks already present in the list
        dependencies.difference_update(benchmark_list)

        return {dep: rev_dependencies[dep] for dep in dependencies}

    def topological_sort(self, recipes_list):
        """
    ...
    def remove_cached_dependencies(self, filtered_results: list) -> list:
        ...
        for bm, value in filtered_results.added_dependencies.items():
            if cache().is_cache_available(bm):
                cache().restore_to_output_folder(bm)
                continue
            dependencies[bm] = value

        filtered_results.added_dependencies = dependencies
        filtered_results.complete_list = sorted(filtered_results.filtered_list + list(dependencies.keys()))
        return filtered_results
