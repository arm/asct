...
class Iperf3SweepBase(RecipeBenchmarkBase):
    ...
    def get_results_df(self):
        ...
        if reporter.output_dir is not None:
            protocol_label = self._protocol_name().upper()
            reporter.plot_iperf3_network_usage(
                df,
                f"{self.name}_network_usage.png",
