...
from collections.abc import Iterable, Sequence  # ruff:ignore[typing-only-standard-library-import]
...
def discover(overwrite: bool = False, frequency: bool = False, output: str | None = None):
    ...
    opts = Options()
    ok = True
    S = system_description(verbose=opts.verbose, opts=opts, frequency=frequency)
    if not S.CMNs:
...
def detect_cpus(update: bool = False):
    ...
    error_msg = "CPU detection failed"
    buf = StringIO()
    try:
        log.info("Detecting CPU locations in the CMN topology. This may take a couple of minutes...")
        ...
        log.debug(f"CPU detection output:\n{buf.getvalue()}")

    # Verify that CPU mappings were detected in the CMN data
    cmn_data = _system_from_json_file(validate_cache=False)
    if not cmn_data or not cmn_data.has_cpu_mappings():
