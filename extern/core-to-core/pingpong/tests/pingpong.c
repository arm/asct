...
thread_pin(thread_state_t *ts, int cpu)
{
    ...
    CPU_ZERO(&cpus);
    CPU_SET(cpu, &cpus);
    /* Thread is created, but might not yet have started and got its tid,
       so set affinity via pthreads API. */
    rc = pthread_setaffinity_np(ts->thread_id, sizeof cpus, &cpus);
    if (rc && o_verbose) {
