...
void handle_args(int argc, char ** argv, args_t * pargs) {
    ...
    static struct option long_options[] = {
        ...
        {"bw-write",            required_argument,  0,      'W'},
#else
...
#ifdef ASCT
                pargs->bw_write = strtoul(optarg, NULL, 0);
                if (pargs->bw_write < 0 || pargs->bw_write >= BW_MODE_MAX_ENUM) {
                    printf("bw-write must be:\n");
                    printf("\t0 : Read only\n");
                    printf("\t1 : Write only\n");
                    printf("\t2 : 3:1 Reads-Writes\n");
                    printf("\t3 : 2:1 Reads-Writes\n");
                    printf("\t4 : 1:1 Reads-Writes\n");
#if NT_STORE_AVAILABLE
                    printf("\t5 : 2:1 Reads-Writes (Non-Temporal Store)\n");
#endif
                    exit(-1);
                }
#else
