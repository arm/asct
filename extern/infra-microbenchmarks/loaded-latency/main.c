...
int main(int argc, char *argv[]) {
    ...
    _Atomic int arrival_count=0;
    _Atomic uint64_t bw_repetitions=0;
    _Atomic uint64_t lat_repetitions=0;
    if (0 != pthread_barrier_init(&barrier, NULL, num_bw_threads + num_lat_threads)) {
...
#ifdef ASCT
            bw_tinfo[bw_thread_num].barrier = &barrier;
            bw_tinfo[bw_thread_num].arrival_count = &arrival_count;
            bw_tinfo[bw_thread_num]._repetitions = &bw_repetitions;
            bw_tinfo[bw_thread_num].total_num_threads = num_bw_threads + num_lat_threads;
#endif
...
#ifdef ASCT
            lat_tinfo[lat_thread_num].barrier = &barrier;
            lat_tinfo[lat_thread_num].arrival_count = &arrival_count;
            lat_tinfo[lat_thread_num]._repetitions = &lat_repetitions;
            lat_tinfo[lat_thread_num].total_num_threads = num_bw_threads + num_lat_threads;
#endif
...
#ifdef ASCT
    double total_bandwidth_bpc = 0.0; // bytes per cycle
#endif
    for (i = 0; i < num_bw_threads; i++) {
...
#ifdef ASCT
        total_bandwidth_bpc += bw_tinfo[i].avg_bw_bpc;
#endif
...
#ifdef ASCT
    double average_latency_cyc = 0.0; // in cycles
#endif
...
#ifdef ASCT
        average_latency_cyc += lat_tinfo[i].avg_latency_cyc;
#endif
        latency_count++;
    }
...
#ifdef ASCT
    average_latency_cyc /= latency_count;
#endif
...
#else
    printf("Total Bandwidth (MB/sec) = %.6f\n", total_bandwidth / 1e6);
    printf("Average Latency (ns) = %.6f\n\n", average_latency);
    printf("Total Bandwidth (B/cycle) = %.6f\n", total_bandwidth_bpc);
    printf("Average Latency (cycle) = %.6f\n\n", average_latency_cyc);
    printf("Bandwidth Repetitions = %lu\n", atomic_load(&bw_repetitions));
    printf("Latency Repetitions = %lu\n\n", atomic_load(&lat_repetitions));
#endif
